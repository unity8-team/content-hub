...
cuc::Transfer* cuc::Hub::create_export_for_type_from_peer(cuc::Type type, cuc::Peer peer)
{
    /* This needs to be replaced with a better way to get the APP_ID */
    QString id = app_id();

    auto reply = d->service->CreateExportForTypeFromPeer(type.id(), peer.id(), id);
    reply.waitForFinished();
    ...
    cuc::Transfer *transfer = cuc::Transfer::Private::make_transfer(reply.value(), this);
    const cuc::Store *store = new cuc::Store{QStandardPaths::writableLocation(QStandardPaths::GenericCacheLocation) + "/" + peer.id() + "/HubIncoming/" + QString::number(transfer->id()), this};
    transfer->setStore(store);
    return transfer;
}
