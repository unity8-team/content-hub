...
struct MockedAppManager : public cua::ApplicationManager
{
    MockedAppManager() : cua::ApplicationManager()
    {
        ...
        ON_CALL(*this, invoke_application_with_session(_,_)).WillByDefault(Return("123"));
    ...
    MOCK_METHOD2(invoke_application_with_session, std::string(const std::string &, const PromptSessionP &));
