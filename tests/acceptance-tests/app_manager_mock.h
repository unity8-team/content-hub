...
struct MockedAppManager : public cua::ApplicationManager
{
    MockedAppManager() : cua::ApplicationManager()
    {
        ...
        ON_CALL(*this, invoke_application_with_session(_,_)).WillByDefault(Return(true));
        ON_CALL(*this, stop_application_with_helper(_)).WillByDefault(Return(true));
    ...
    MOCK_METHOD2(invoke_application_with_session, bool(const std::string &, const PromptSessionP &));
    MOCK_METHOD1(stop_application_with_helper, bool(const std::string &));
